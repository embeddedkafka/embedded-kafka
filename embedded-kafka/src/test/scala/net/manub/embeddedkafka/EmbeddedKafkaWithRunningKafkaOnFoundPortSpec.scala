...
class EmbeddedKafkaWithRunningKafkaOnFoundPortSpec
  ...
  "the withRunningKafkaOnFoundPort method" should {
    ...
    "start and stop Kafka, Zookeeper, and Schema Registry successfully on non-zero ports" in {
      val userDefinedConfig = EmbeddedKafkaConfig(kafkaPort = 12345, zooKeeperPort = 12346, schemaRegistryPort = Some(12347))
      val actualConfig = withRunningKafkaOnFoundPort(userDefinedConfig) { actualConfig =>
        actualConfig shouldBe userDefinedConfig
        everyServerIsAvailable(actualConfig)
        actualConfig
    ...
    "start and stop multiple Kafka and Zookeeper successfully on arbitrary available ports" in {
      ...
      val actualConfig1 = withRunningKafkaOnFoundPort(userDefinedConfig) { actualConfig1 =>
        ...
        val actualConfig2 = withRunningKafkaOnFoundPort(userDefinedConfig) { actualConfig2 =>
          ...
          allConfigs.map(config => EmbeddedKafkaConfigImpl(kafkaPort = 0, zooKeeperPort = 0, schemaRegistryPort = None, config.customBrokerProperties, config.customProducerProperties, config.customConsumerProperties)).distinct should have size 1
          actualConfig2
        }
